...
pipeline {
    environment {
        ...
        ORGANIZATION = "ons"
        TEAM = "sbr"
        MODULE_NAME = "sbr-data-validator"
    ...
    sshagent(credentials: ["sbr-$DEPLOY_DEV-ci-ssh-key"]) {
        withCredentials([string(credentialsId: "sbr-hbase-node", variable: 'HBASE_NODE')]) {
            sh '''
                ...
                scp ${WORKSPACE}/target/scala-2.11/sbr-data-validator-assembly*.jar sbr-$DEPLOY_DEV-ci@$HBASE_NODE:$MODULE_NAME/lib/
                echo "Successfully copied jar file to $MODULE_NAME/lib directory on $HBASE_NODE"
                ssh sbr-$DEPLOY_DEV-ci@$HBASE_NODE hdfs dfs -put -f $MODULE_NAME/lib/sbr-data-validator-assembly*.jar hdfs://prod1/user/sbr-$DEPLOY_DEV-ci/lib/
